...
`)
	...
	// Both the path-only repo and the local-only repo should be present
	if len(r.Repos) != 2 {
		t.Fatalf("expected 2 repos, got %d", len(r.Repos))
	}
	found := false
...
func TestLocalOnlyWithoutBundle(t *testing.T) {
	// Local-only entry without a bundle should fail validation.
	s := setupStore(t)
	writeFile(t, s.Root, "registry.toml", `
[[repos]]
path = "/tmp/project"
...
`)
	_, err := Load(s)
	if err == nil {
		t.Fatal("expected error for local-only entry without bundle")
	}
}
